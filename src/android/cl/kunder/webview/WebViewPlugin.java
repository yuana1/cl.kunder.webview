...
public class WebViewPlugin extends CordovaPlugin {
    ...
    private static Stack<CallbackContext> subscribeCallbackContexts = new Stack<CallbackContext>();
    private static CallbackContext subscribeExitCallbackContext = null;
    private static JSONObject subscribeCallbackData = null;
    private static int webViewSize = 0;
        ...
        if (action.equals("show") && args.length() > 0) {
            ...
            if (!"".equals(url)) {
                ...
                synchronized (WebViewPlugin.class) {
                    ...
                    subscribeCallbackContexts.push(null);
                    webViewSize++;
                }
            ...
            try {
                subscribeCallbackData = args.getJSONObject(0);
            } catch (Exception e) {
                subscribeCallbackData = new JSONObject();
            }
            hideWebView();
            JSONObject r = new JSONObject();
            r.put("responseCode", "ok");
            ...
            synchronized (WebViewPlugin.class) {
                if (subscribeCallbackContexts.size() == webViewSize)
                    subscribeCallbackContexts.pop();
                subscribeCallbackContexts.push(callbackContext);
            }
    ...
    private void hideWebView() {
        ...
        synchronized (WebViewPlugin.class) {
            webViewSize--;
            int size = subscribeCallbackContexts.size();
            if (size != 0) {
                subscribeCallbackContext = subscribeCallbackContexts.pop();
                if (subscribeCallbackContext != null) {
                    subscribeCallbackContext.success(subscribeCallbackData);
                }
            }
        }
